...
O CampusConnect usa a **Groq API** com o modelo **Llama 3 8B** para moderação de conteúdo e sugestão de tags. A Groq oferece um tier gratuito com 30 RPM / 14.400 RPD, suficiente para desenvolvimento e testes.
...
├── groq-client.ts          # Cliente HTTP genérico para a API Groq
...
- **GroqClient**: Cliente HTTP que encapsula a comunicação com a API do Groq. Responsável por autenticação, headers e chamadas HTTP.
...
- `GroqClient`: `Backend/src/infrastructure/ai/groq-client.ts` — cliente HTTP para a API Groq.
...
- A implementação usa `fetch()` nativo, sem dependência de SDK externo.
